...
// controls the minimum size of the arc
  ...
  Timer? timer;
  static const int timePortion = 10; // 40 milliseconds for each increment
  ...
  void initState() {
    ...
    // TODO(khaled): clarify this equation and extract a constant
    // controls the smoothness of the arc animation
    arcIncrement = 2 *
        2 *
        (100 - minimumArcSize + (100 * 0.1)) *
        (1 - staticStatePercent) *
        (timePortion / widget.duration);
  ...
  _startRotation() {
    ...
    timer =
        Timer.periodic(const Duration(milliseconds: timePortion), (Timer t) {
      if (mounted) {
        setState(() {
          if (!reverseFlag) {
            if (!staticState) {
...
class AwesomeLoaderController {
  _AwesomeLoaderState _awesomeLoaderState = _AwesomeLoaderState();
